    ...
    public function retrievePaymentResult(string $token): array
    {
        try {
            ...
            if ($result->getStatus() === 'success' && $result->getPaymentStatus() === 'SUCCESS') {
                ...
                // Use only methods that actually exist
                $rawData = [
                    'status' => $result->getStatus(),
                    ...
                    'posOrderId' => $result->getPosOrderId(),
                    'connectorName' => $result->getConnectorName(),
                    'phase' => $result->getPhase(),
                    'cardToken' => $result->getCardToken(),
                    'cardUserKey' => $result->getCardUserKey(),
                    'merchantCommissionRate' => $result->getMerchantCommissionRate(),
                    'merchantCommissionRateAmount' => $result->getMerchantCommissionRateAmount(),
                    'iyziCommissionRateAmount' => $result->getIyziCommissionRateAmount(),
                    'iyziCommissionFee' => $result->getIyziCommissionFee(),
